...
def pcl_callback(pcl_msg):
    ...
    mymeank = 10 
    outlier_filter.set_mean_k(mymeank)
    mystddevmulth = 0.01 
    outlier_filter.set_std_dev_mul_thresh(mystddevmulth)
    ...
    myminclsz = 100   
    mymaxclsz = 1500  
    ec.set_ClusterTolerance(myclstol)
    ec.set_MinClusterSize(myminclsz)
    ec.set_MaxClusterSize(mymaxclsz)
    ...
    # and assign a color corresponding to each segmented object in scene
...
def pr2_mover(object_list):
    ...
    dict_dropbox = {}
    for dpbx in dropbox_param:
        dict_dropbox[dpbx['name']] = dpbx['position']
    ...
    for obj in object_list_param:
        ...
        # set object_name (derived from the data in pick_list_n.yaml)
        object_name = String()
        ...
        # if the detected_object.label is equal to object_name, 
...
if __name__ == '__main__':
    ...
    rospy.init_node('pr2', anonymous=True) # clustering, False  # object_recognition, True
    ...
    object_markers_pub = rospy.Publisher("/object_markers", Marker, queue_size=1) #qu_sz 10
    detected_objects_pub = rospy.Publisher("/detected_objects", DetectedObjectsArray, queue_size=1)  #qu_sz 10
